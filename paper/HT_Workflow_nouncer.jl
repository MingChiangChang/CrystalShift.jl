...
RANK = 4
STRIPE_IDX = 336
amorphous = Wildcard([], [], [], "amorphous", Lorentz(), [], [])
...
function get_probabilities(results::AbstractVector,
    ...
    for i in 1:length(results)
        ...
        prob[i] = approximate_negative_log_evidence(results[i], θ, x, y, std_noise, full_mean_θ, full_std_θ, LeastSquares())
    end
...
@time begin 
    ...
    for phase in unique_phases
        ...
        for i in 1:free_param_num
            ...
            plot!(0.01(x.-115), 100((param./origin_param).-1), label="$(simple_phase_name[phase]) $(param_label)", linewidth=2, markershape=:circle)
